    ...
    if (!subscribeButton.classList.contains("registrationLink")) {
      try {
        ...
        const res = await fetch(
          `https://www.finbrowser.io/api/lists/${list_id}/list_change_subscribtion_status/`,
  ...
  deleteArticlesButton.forEach((closeButton) => {
    ...
    closeButton.addEventListener("click", async () => {
      try {
        ...
        const res = await fetch(
          `https://www.finbrowser.io/api/lists/${list_id}/delete_article_from_list/${article_id}/`,
    ...
    .addEventListener("click", async () => {
      try {
        const list_id = document
          ...
          .id.replace("list_detail_for_", "");
        const res = await fetch(
          `https://www.finbrowser.io/api/lists/${list_id}/`,
          get_fetch_settings("DELETE")
        );
        if (!res.ok) {
          showMessage("Error: Network request failed unexpectedly!", "Error");
          ...
          const context = await res.json();
          showMessage(context, "Remove");
          window.location.href = "https://www.finbrowser.io/lists";
        }
      } catch (e) {
        // showMessage("Error: Unexpected error has occurred!", "Error");
      }
  ...
  document
    ...
    .addEventListener("keyup", async function (e) {
      ...
      if (search_term && search_term.replaceAll(/\s/g, "") != "") {
        ...
        try {
          const res = await fetch(
            `https://www.finbrowser.io/api/sources/?list_search=${search_term}&list_id=${list_id}`,
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      if (selected_sources.length) {
        ...
        try {
          const res = await fetch(
            `https://www.finbrowser.io/api/lists/${list_id}/add_sources_to_list/${selected_sources}/`,
            get_fetch_settings("POST")
          );
          if (!res.ok) {
            showMessage("Error: Network request failed unexpectedly!", "Error");
...
document.querySelectorAll(".rankingStar").forEach((star) => {
  star.addEventListener("click", async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://www.finbrowser.io/api/list_ratings/`, {
        method: "POST",
        headers: {
          "X-CSRFToken": getCookie("csrftoken"),
          Accept: "application/json",
