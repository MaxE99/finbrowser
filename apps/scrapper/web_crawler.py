...
def crawl_thegeneralist(articles):
    ...
    try:
        ...
        for article in soup.find_all("div", class_="w-dyn-item"):
            ...
            title = html.unescape(
                info_container.find("div", class_="single-article-title v2").text
            )
...
def crawl_meritechcapital(articles):
    ...
    try:
        page = requests.get(
            "https://www.meritechcapital.com/insights",
            headers={
                "User-Agent": "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36 QIHU 360SE"
            },
            timeout=10,
        ...
        for article in soup.find_all("a", class_="BlogLandingPage_card__FtA_n"):
            ...
            pub_date = datetime.strptime(
                pub_date.replace(",", "").replace(" | ", ""), "%b %d %Y"
            )
            link = f"https://www.meritechcapital.com{article['href']}"
            title = html.unescape(
                body_container.find("h3", class_="BlogPostCard_title__dnPww").text
            )
...
def crawl_stockmarketnerd(articles):
    ...
    try:
        page = requests.get(
            "https://stockmarketnerd.beehiiv.com/",
            headers={
                "User-Agent": "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36 QIHU 360SE"
            },
            timeout=10,
