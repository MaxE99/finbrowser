...
const notificationButton = document.querySelector(
  ".stockActionContainer .notificationButton"
);
  ...
  .addEventListener("click", (e) => {
    if (!e.target.classList.contains("openAuthPrompt")) {
      ...
      document.querySelector(
        ".stockMenuWrapper .addStockContainer"
      ).style.display = "block";
            ...
            for (let i = 0, j = lists_status.length; i < j; i++) {
              if (lists_status[i] != initial_lists_status[i]) {
                if (initial_lists_status[i] == false) {
                  ...
                  removeStocksFromportfolio.push(
                    list_ids[i].replace("id_", "")
                  );
                }
              }
            }
...
document
  .querySelector(
    ".stockMenuWrapper .addStockContainer .formHeaderContainer .fa-times"
  )
...
document
  .querySelectorAll(
    ".stockMenuWrapper .portfolioSelectionContainer .portfolioContainer"
  )
